...
var bf_spinner = function( spinner_el, spinner_opts ) {
	var s = false;
	var started = null;
	var target;
	
	
	var generate_opts = function() {
		
		// make a big ol' spinner based on viewport height
		
		var x = Math.floor((document.viewport.getHeight() / 3) / 3.5);
		
		var tmp = {
		  lines: 8, // The number of lines to draw
		  length: x, // The length of each line
		  width: Math.floor(0.66*x), // The line thickness
		  radius: Math.floor(0.9*x), // The radius of the inner circle
	...
	var opts = spinner_opts || generate_opts();

	if ( spinner_el && $(spinner_el) )
	{
		target = $(spinner_el);
	}
	else
	{
		try {
			// great big spinner element to cover the page
			target = new Element('div', { 'class': 'spinner_container', id: 'spinner_' + Math.floor(Math.random() * 100) });
			document.body.insert( {top: target} );
			var styles = 'display:none;width:100%;position:fixed;left:0;height:' + document.viewport.getHeight() + 'px;z-index:9999;top:0;';
	...
	return {
		...
		start: function() {
			...
			if (s)
			{
				window.document.body.addClassName('spinner-activated');
				s = s.spin(target);
				return true;
			}

			target.on('click', function(e) { if (started) e.stop(); });

			s = new Spinner(opts).spin(target);
		},
		stop: function() {
			try {
				window.document.body.removeClassName('spinner-activated');
				target.hide();
				started = false;
				s.stop();
			} catch(err) { }
